...
def main():
    ...
    df_grafik_rabot['Planned start time'] = pd.to_datetime(df_grafik_rabot['Planned start time']).replace('1970-01-01',
                                                                                                          'NaT')
    df_grafik_rabot['Planned finish time'] = pd.to_datetime(df_grafik_rabot['Planned finish time']).replace(
        '1970-01-01', 'NaT')
    df_grafik_rabot['Actual start time'] = pd.to_datetime(df_grafik_rabot['Actual start time']).replace('1970-01-01',
                                                                                                        'NaT')
    df_grafik_rabot['Actual finish time'] = pd.to_datetime(df_grafik_rabot['Actual finish time']).replace('1970-01-01',
                                                                                                          'NaT')
    ...
    df_tabl_stavki['Дата нового условия ставки (час)'] = pd.to_datetime(
        df_tabl_stavki['Дата нового условия ставки (час)'].replace('0', np.nan)).replace('1970-01-01', 'NaT')
    ...
    df_tabl_stavki['Нач. дата расчета бонуса нового сотрудника'] = pd.to_datetime(
        df_tabl_stavki['Нач. дата расчета бонуса нового сотрудника']).replace('1970-01-01', 'NaT')
    df_tabl_stavki['Оконч. дата расчета бонуса нового сотрудника'] = pd.to_datetime(
        df_tabl_stavki['Оконч. дата расчета бонуса нового сотрудника'])
    ...
    df = df[['Месяц', 'Worker id', 'Worker username', 'Worker nickname',
             ...
             'Бонус приведи друга', 'Штраф', 'Аванс', 'Всего удержано']]
