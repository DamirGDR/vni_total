...
def main():
    ...
    # for index, row in df_vyezd.iterrows():
    #     if not pd.isnull(df_vyezd.loc[index]['Дата выезда 1']):
    #         res_for_workers.loc[(res_for_workers['Worker id'] == df_vyezd.loc[index]['Worker id']) & \
    #                             (res_for_workers['Date'] == df_vyezd.loc[index]['Дата выезда 1']), 'Ставка за час'] = \
    #             float(df_vyezd.loc[index]['Ставка за выезд в другие города'])
    #     if not pd.isnull(df_vyezd.loc[index]['Дата выезда 2']):
    #         res_for_workers.loc[(res_for_workers['Worker id'] == df_vyezd.loc[index]['Worker id']) & \
    #                             (res_for_workers['Date'] == df_vyezd.loc[index]['Дата выезда 2']), 'Ставка за час'] = \
    #             float(df_vyezd.loc[index]['Ставка за выезд в другие города'])
    #     if not pd.isnull(df_vyezd.loc[index]['Дата выезда3']):
    #         res_for_workers.loc[(res_for_workers['Worker id'] == df_vyezd.loc[index]['Worker id']) & \
    #                             (res_for_workers['Date'] == df_vyezd.loc[index]['Дата выезда3']), 'Ставка за час'] = \
    #             float(df_vyezd.loc[index]['Ставка за выезд в другие города'])
    #     if not pd.isnull(df_vyezd.loc[index]['Дата выезда4']):
    #         res_for_workers.loc[(res_for_workers['Worker id'] == df_vyezd.loc[index]['Worker id']) & \
    #                             (res_for_workers['Date'] == df_vyezd.loc[index]['Дата выезда4']), 'Ставка за час'] = \
    #             float(df_vyezd.loc[index]['Ставка за выезд в другие города'])
    #     if not pd.isnull(df_vyezd.loc[index]['Дата выезда5']):
    #         res_for_workers.loc[(res_for_workers['Worker id'] == df_vyezd.loc[index]['Worker id']) & \
    #                             (res_for_workers['Date'] == df_vyezd.loc[index]['Дата выезда5']), 'Ставка за час'] = \
    #             float(df_vyezd.loc[index]['Ставка за выезд в другие города'])
    #
    # # Учет дат выезда. Конец
